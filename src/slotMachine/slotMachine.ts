...
export class SlotMachine {
  ...
  constructor() {
    ...
    this.balance = 1000;
    this.winLines = [];
    this.winningSymbols = Array(SlotMachine.REELS)
      .fill(null)
      .map(() => []);
  }
  ...
  private async spin() {
    ...
    const totalSpinTime =
      SlotMachine.REEL_SPIN_TIME +
      (SlotMachine.REELS - 1) * SlotMachine.REEL_STOP_DELAY;
    const startTime = Date.now();

    const spinPromises = this.reels.map(
      (reel, index) =>
  ...
  private highlightWinningSymbols(row: number) {
    for (let i = 0; i < SlotMachine.REELS; i++) {
      const symbolContainer = this.reels[i].container.children[
        row
      ] as Container;
      const highlight = new Graphics();
      highlight.lineStyle(5, 0xffd700, 1);
      highlight.drawRect(
        -SlotMachine.SYMBOL_SIZE / 2,
        -SlotMachine.SYMBOL_SIZE / 2,
        SlotMachine.SYMBOL_SIZE,
        SlotMachine.SYMBOL_SIZE
      );
